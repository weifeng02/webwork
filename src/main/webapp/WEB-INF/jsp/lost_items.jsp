<%--
  失物列表页面 (lost_items.jsp)
  功能：展示所有失物信息，支持按类别、地点、状态筛选
  访问路径：/lost/list
  数据：lostItems - 失物列表
  参数：category, location, status - 筛选条件
--%>
<!-- 引入公共头部模板，设置页面标题为"失物列表" -->
<jsp:include page="header.jsp">
    <jsp:param name="title" value="失物列表"/>
</jsp:include>
...
<div class="filter-bar">
    <!-- 筛选表单：GET方法提交到 /lost/list，保持筛选条件在URL中便于分享和分页 -->
    <form action="${pageContext.request.contextPath}/lost/list" method="get" style="display:flex;gap:12px;flex-wrap:wrap;">
        ...
        <select name="status">
            ...
            <option value="pending" ${param.status == 'pending' ? 'selected' : ''}>待找回</option>
            <option value="found" ${param.status == 'found' ? 'selected' : ''}>已找回</option>
        </select>
        <!-- 筛选提交按钮：触发筛选条件应用 -->
    ...
    <c:forEach var="item" items="${lostItems}">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <!-- 状态标签：根据状态值显示不同样式（pending/found） -->
